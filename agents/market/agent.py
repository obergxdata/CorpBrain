...
class Market:
    ...
    def __init__(self):
        ...
        self.history: History = History(window=256)
        self.tick: int = 0
    ...
    def step(self):
        ...
        for person in self.people:
            person.get_paid(SIM_CONFIG.base_salary)
    ...
    @property
    def avg_price(self):
        return sum([corp.price for corp in self.corporations]) / len(self.corporations)

    @property
    def min_max_price(self):
        prices = [corp.price for corp in self.corporations]
    ...
    def init_people(self):
        for _ in range(0, SIM_CONFIG.nr_of_people):
            person = Person(
                mpc=PERSON_SEED.mpc,
                balance=PERSON_SEED.balance,
                latest_pay=PERSON_SEED.balance,
            )
